...
const ManualCropModal: React.FC<ManualCropModalProps> = ({ imageUrl, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 backdrop-blur-md p-10 animate-in fade-in duration-500">
      <div className="bg-[#07050f] border border-purple-500/20 rounded-[2.5rem] w-full max-w-5xl h-[85vh] overflow-hidden flex shadow-[0_0_100px_rgba(168,85,247,0.2)]">
        ...
        <div className="flex-1 flex flex-col bg-black relative">
          <div className="flex-1 relative flex items-center justify-center p-8 overflow-hidden group">
            ...
            <div className="absolute bottom-6 left-6 text-[9px] font-black uppercase tracking-widest text-purple-400/60 bg-black/60 px-4 py-2 rounded-full border border-purple-500/10 backdrop-blur-xl">
        ...
        <div className="w-80 border-l border-white/5 flex flex-col p-10 bg-gradient-to-b from-[#0d0a1a] to-[#07050f]">
          <div className="flex justify-between items-center mb-10 shrink-0">
            <h3 className="text-[11px] font-black uppercase tracking-[0.3em] text-slate-400">手动框选搜索</h3>
            <button onClick={onClose} className="p-2 hover:bg-white/5 rounded-xl text-gray-500 hover:text-purple-400 transition-all">
          ...
          <div className="flex-1 overflow-y-auto pr-2 no-scrollbar space-y-10">
             <div className="space-y-6">
                <label className="text-[10px] font-black text-slate-600 uppercase tracking-widest">选择目标类型</label>
          ...
          <div className="mt-auto space-y-4 shrink-0 pt-10">
             <button className="w-full py-4 bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-500 hover:to-indigo-600 rounded-2xl font-black text-[11px] text-white uppercase tracking-[0.2em] shadow-[0_0_30px_rgba(168,85,247,0.3)] transition-all flex items-center justify-center gap-3">
               {ICONS.Search} 立即开始搜索
             </button>
             <button onClick={() => setActiveCrop(null)} className="w-full py-4 bg-white/5 hover:bg-white/10 border border-white/5 rounded-2xl text-[10px] font-black uppercase tracking-widest text-gray-500 transition-all">
               重置选择
             </button>
          </div>
        </div>
