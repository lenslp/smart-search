...
const SurveillanceModal: React.FC<SurveillanceModalProps> = ({ selectedItems: initialItems, onClose, onDeploy }) => {
  ...
  return (
    <div className="fixed inset-0 z-[160] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6 animate-in fade-in zoom-in-95 duration-300">
      <div className="bg-[#05050a] border border-purple-500/30 rounded-[3rem] w-full max-w-6xl h-[80vh] overflow-hidden flex shadow-[0_0_120px_rgba(139,92,246,0.3)]">
        
        {/* Left: Sample Pool */}
        <div className="w-80 border-r border-white/5 bg-gradient-to-b from-purple-900/10 to-transparent p-8 flex flex-col">
          <div className="flex-1 overflow-y-auto no-scrollbar">
            <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] mb-6 flex items-center justify-between">
              待布控样本
              <span className="text-purple-500">{localItems.length}</span>
            </h3>
            <div className="grid grid-cols-2 gap-3 pb-8">
              ...
              <button 
                ...
                className="aspect-square rounded-2xl border-2 border-dashed border-purple-500/20 flex flex-col items-center justify-center gap-1.5 text-purple-400 hover:bg-purple-500/5 hover:border-purple-500/50 transition-all group"
              >
                <div className="group-hover:scale-110 transition-transform">{ICONS.Plus}</div>
                <span className="text-[8px] font-black uppercase tracking-widest">添加图片</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div className="flex-1 flex flex-col p-10 overflow-hidden">
          <div className="flex justify-between items-start mb-8 shrink-0">
            ...
            <button onClick={onClose} className="p-3 hover:bg-white/5 rounded-2xl text-slate-500 hover:text-purple-400 transition-all border border-transparent hover:border-purple-500/20">
          ...
          <div className="flex-1 overflow-y-auto pr-4 custom-scrollbar space-y-10 pb-10">
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <button onClick={() => addCondition('text')} className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[9px] font-black text-slate-400 hover:text-purple-400 transition-all uppercase tracking-widest flex items-center gap-2">
                    {ICONS.Plus} 语义词
                  </button>
                  <button onClick={() => addCondition('image')} className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-[9px] font-black text-slate-400 hover:text-purple-400 transition-all uppercase tracking-widest flex items-center gap-2">
                    {ICONS.Plus} 视觉特征
                  </button>
                </div>
              </div>
            ...
            <section className="space-y-4">
              ...
              <div className="grid grid-cols-4 gap-2 p-3 bg-black/40 border border-white/5 rounded-2xl">
                  {['持续布控', '日间', '夜间', '自选'].map(t => (
                    <button key={t} className={`py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${t === '持续布控' ? 'bg-purple-600 border-purple-600 text-white shadow-lg' : 'border-white/5 text-slate-500 hover:border-purple-500/30'}`}>{t}</button>
                  ))}
              </div>
            </section>
            ...
            <section className="space-y-4">
              ...
              <button className="w-full p-4 h-auto min-h-[74px] bg-black/40 border border-white/5 rounded-2xl flex items-center justify-between group hover:border-purple-500/40 transition-all">
                <div className="flex items-center gap-3">
                  <div className="p-2 bg-purple-500/10 text-purple-500 rounded-xl group-hover:bg-purple-600 group-hover:text-white transition-all">{ICONS.Map}</div>
                  <div className="text-left">
                    <div className="text-[10px] font-black text-slate-300 uppercase tracking-widest">全域智能感知覆盖</div>
                    <div className="text-[8px] font-bold text-slate-600 uppercase">已激活 12 个关联点位及边缘计算单元</div>
                  </div>
                </div>
                <div className="text-slate-600 group-hover:text-purple-400 transition-all scale-75">{ICONS.Down}</div>
              </button>
            </section>
          </div>
          ...
          <div className="pt-8 flex gap-4 shrink-0 border-t border-white/5 mt-auto">
            <button onClick={onClose} className="flex-1 py-4 bg-white/5 hover:bg-white/10 border border-white/10 rounded-2xl text-[10px] font-black text-slate-500 uppercase tracking-[0.3em] transition-all">
              取消返回
            </button>
            <button 
              onClick={() => onDeploy({ conditions, items: localItems })}
              className="flex-[2] py-4 bg-gradient-to-r from-purple-600 to-indigo-700 hover:from-purple-500 hover:to-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-[0.3em] shadow-[0_15px_30px_rgba(139,92,246,0.3)] transition-all flex items-center justify-center gap-3 group"
            >
